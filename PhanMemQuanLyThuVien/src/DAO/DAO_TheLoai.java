...
public class DAO_TheLoai {
    ...
    public ArrayList<DTO_TheLoai> TimKiemTheoMa(String MaTheLoai) {
        ArrayList<DTO_TheLoai> DanhSachTheLoai = new ArrayList<>();
        String TruyVan = "Select MaTheLoai, TenTheLoai From TheLoai Where MaTheLoai = '" + MaTheLoai + "' And TrangThai = true";
        try {
            PreparedStatement = new MySQLConnection().Connection.prepareStatement(TruyVan);
            ResultSet = PreparedStatement.executeQuery();
            while (ResultSet.next()) {
    ...
    public Boolean Them(DTO_TheLoai TheLoai) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Thêm thể loại mới không thành công !", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
        }
        return false;
    }
